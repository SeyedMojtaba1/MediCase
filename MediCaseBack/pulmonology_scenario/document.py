...
ASTHMA_REF = """
## 1. ChiefComplaint (شکایت اصلی)
**راهنما:** شکایت اصلی باید بر اساس سناریو انتخاب شود اما بیان آن متنوع باشد.
* **[Scenario: mild_allergic]:** شکایت از سرفه‌های خشک، تنگی نفس خفیف فصلی، یا سنگینی قفسه سینه در مواجهه با گرد و خاک. علائم معمولاً همیشگی نیستند.
* **[Scenario: severe_uncontrolled]:** شکایت از تنگی نفس مداوم، بیداری‌های مکرر شبانه به دلیل سرفه یا خس‌خس، و عدم تاثیر کافی اسپری‌های معمول. بیمار احساس کلافگی دارد.
* **[Scenario: exercise_induced]:** شکایت متمرکز بر فعالیت است؛ تنگی نفس، سرفه یا سوزش ریه که صرفاً حین یا بعد از ورزش (دویدن، فوتبال) رخ می‌دهد و در استراحت وجود ندارد.

## 2. PresentIllness (بیماری فعلی)
* **Onest & Course:**
    * **[mild_allergic]:** شروع تدریجی و نوسانی. ممکن است بیمار بگوید "چند هفته است که باز شروع شده" یا "هر بهار اینطور می‌شوم".
    * **[severe_uncontrolled]:** سیر پیشرونده یا مزمن بدون دوره بهبودی کامل. "مدت‌هاست درگیرم و اخیراً بدتر شده‌ام".
    * **[exercise_induced]:** شروع حاد حین فعالیت. "۱۰ دقیقه که می‌دوم شروع می‌شود".
* **Dyspnea Characteristics:**
    * **[mild_allergic]:** تنگی نفس با فعالیت سنگین یا تماس با آلرژن. در استراحت اغلب خوب است.
    * **[severe_uncontrolled]:** تنگی نفس حتی با فعالیت کم (راه رفتن در خانه) یا در استراحت. صحبت کردن ممکن است دشوار باشد.
    * **[exercise_induced]:** تنگی نفس فقط در اوج فعالیت ورزشی.
* **Cough & Sputum:**
    * **[mild_allergic]:** سرفه خشک، تحریکی، بیشتر در شب یا صبح زود. خلط ندارد یا اندک و شفاف است.
    * **[severe_uncontrolled]:** سرفه‌های خسته‌کننده در طول شبانه‌روز. ممکن است خلط چسبناک و غلیظ داشته باشد.
    * **[exercise_induced]:** سرفه خشک که بعد از قطع ورزش شروع می‌شود.

## 3. PastMedicalHistory (سابقه پزشکی)
* **Co-morbidities:**
    * **[mild_allergic]:** سابقه حساسیت فصلی یا بدون سابقه قبلی.
    * **[severe_uncontrolled]:**  سابقه سینوزیت مزمن، پولیپ بینی، یا ریفلاکس معده (GERD) یا بدون سابقه.
    * **[exercise_induced]:** معمولاً سابقه پزشکی خاصی ندارد و فرد سالم است.

## 7. SocialHistory (سابقه اجتماعی)
* **[mild_allergic]:** محیط زندگی ممکن است دارای محرک مثل سگ یا گربه یا پرنده باشد.ممکنه بدون سابقه آلرژی باشد .
* **[severe_uncontrolled]:** محیط زندگی ممکن است دارای محرک مثل سگ یا گربه یا پرنده باشد.ممکنه بدون سابقه آلرژی باشد
* **[exercise_induced]:** معمولاً افراد فعال، دانش‌آموزان یا ورزشکاران هستند.

## 8. ReviewOfSystems (ROS)
* **General:** در نوع شدید ممکن است خستگی ذکر شود. تب و لرز وجود ندارد.
* **HEENT:** در نوع آلرژیک، علائم چشمی (خارش، قرمزی) یا بینی (آبریزش، گرفتگی) شایع است.
* **Respiratory:** تنگی نفس، خس‌خس (Wheezing)، سرفه. خلط خونی (Hemoptysis) مورد انتظار نیست.
* **Cardiac:** احساس فشار روی قفسه سنه ممکن است باشد. تورم پا وجود ندارد.
"""
