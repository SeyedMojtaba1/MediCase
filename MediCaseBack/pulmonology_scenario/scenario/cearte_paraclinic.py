...
class LabDataGenerator:
    ...
    # --- منابع داده برای تولید Patient Profile (به روز شده) ---
    RANDOM_DATA_LISTS = {
        ...
        # مشاغل بر اساس منطق جدید تفکیک شدند
        "occupations_male": [
    ...
    def __init__(self):
        self.random = random
        # 1. Initialization: Define Case Type, which implicitly sets Effusion Status
    ...
    # --- توابع جدید برای Patient Profile ---

    def _select_occupation(self, gender, age_str):
        age = int(age_str.split()[0])
        
        # Rule 1: Retirement (بالای 65 سال)
        if age > 65:
            return self.random.choice(self.RANDOM_DATA_LISTS["occupations_retirement"])
        
        # Rule 2: Transition (55 تا 65 سال)
        if 55 <= age <= 65:
            if self.random.random() < 0.5: # 50% Retirement
                return self.random.choice(self.RANDOM_DATA_LISTS["occupations_retirement"])
            # 50% Gender-specific
            else:
                if gender == "مرد":
    ...
    def _select_place_of_residence(self, place_of_birth):
        # 70% chance to choose a nearby city
        if self.random.random() < 0.7:
            nearby_cities = self.RANDOM_DATA_LISTS["city_proximity"].get(place_of_birth, [])
            # If nearby list is available, choose from it
            if nearby_cities:
                return self.random.choice(nearby_cities)
            
        # 30% chance (or if nearby list is unavailable/empty) choose from all cities
    ...
    def _select_marital_status(self, gender, age_str):
        ...
        # Define probability of being 'متأهل' (married)
        prob_married = 0
        ...
        # 1. Determine if the patient is likely "Married" or "Not Married"
        if self.random.random() < prob_married:
            # If likely married, choose between 'متأهل' and 'همسر متوفی' (mostly 'متأهل')
            return self.random.choices(["متأهل", "همسر متوفی"], weights=[90, 10], k=1)[0]
        else:
            # If likely not married, choose between 'مجرد' and 'مطلقه' (mostly 'مجرد')
            return self.random.choices(["مجرد", "مطلقه"], weights=[70, 30], k=1)[0]

    def _generate_personal_information(self):
        # 1. Choose Gender & Age (Adults 40-75 for likely pneumonia case)
        ...
        # 3. Choose Occupation (New Logic)
        occupation = self._select_occupation(gender, age_str)
        
        # 4. Choose Locations (New Logic)
    ...
    def _generate_chief_complaint(self):
        if self.severity_level == "SEVERE":
            # اگر شدت بالا باشد، شکایت اصلی شدیدتر است.
            ...
            # اگر شدت پایین باشد، شکایت اصلی خفیف‌تر است.
            return self.random.choice([
    ...
    # --- توابع تعیین وضعیت اصلی (بر اساس منطق جدید) ---
    def _determine_case_type(self):
        # LOBAR_PE_PRESENT (15%), LOBAR_PE_ABSENT (60%), INTERSTITIAL (25%)
    ...
    def _determine_severity(self):
        # Check for Tachycardia (>100), Tachypnea (>20), Fever (>38.0), or Hypoxia (<95% based on SpO2 dist)
        # Note: Vital sign values are strings that need parsing
    ...
    # --- 2. General Appearance Generators (بر اساس شدت) ---
    def _gen_loc(self):
        if self.severity_level == "MILD":
    ...
    # --- 4. Respiratory System Generators (بر اساس Case Type و Severity) ---
    ...
    # --- 5. Cardiovascular, Abdominal, Neuro, MSK ---
    ...
    # --- 6. Paraclinic Generators (Imaging & Procedures بر اساس Effusion Status) ---
    def _gen_cxr(self):
    ...
    # --- سایر توابع پاراکلینیک (بدون تغییر) ---
    ...
    def generate_paraclinic_case(self):
        ...
        """
        # 1. Generate Vitals (initial values) and determine severity
        self.vital_signs = {
        ...
        # 1b. Determine Severity based on generated Vitals
        self.severity_level = self._determine_severity()
        
        # 1c. Overwrite GCS to match determined severity
        self.vital_signs["GCS"] = self._generate_gcs_value()
        
        # 1d. Generate Patient Profile (now including chief complaint)
        personal_info = self._generate_personal_information()
        chief_complaint = self._generate_chief_complaint()
        
        # 2. Pre-calculate Physical Exam Findings based on CASE_TYPE and SEVERITY
        ...
        # 3. Construct the Exact JSON Structure
        data = {
            "patient_profile": {
                "personal_information": personal_info,
            },
            "physical_exam": {
                "vital_signs": self.vital_signs, # Kept here for PE consistency
                "general_appearance": {
...
if __name__ == "__main__":
    ...
    final_case = generator.generate_paraclinic_case()
