...
def history_taking_creator(target_disease):
    ...
    prompt_template = PromptTemplate(
        ...
        1.  بخش شرح حال (History Taking): پاسخ‌ها باید به -فارسی روان- ، به صورت اول شخص مفرد (از زبان بیمار) و توصیفی تولید شوند. از تکرار تحلیل‌های پزشکی، درصدها یا بازه‌های زمانی خودداری کنید. از جملات و کلمات عامیانه استفاده کن و از بکار بردن اصطلاحات پزشکی خودداری کن.
        2.  سوالات با پاسخ بله و خیر را فقط با بله یا خیر جواب بده 
        3.  حذف کامل تحلیل و درصد: به هیچ وجه هیچ‌کدام از اطلاعات آماری، درصدها (مانند 80%، 20%)، بازه‌های زمانی (مانند 1 تا 7 روز) یا تحلیل‌های پزشکی (مانند risk factor) موجود در «متون رفرنس» را در خروجی JSON تکرار نکنید.
        4.  تولید مقادیر واقعی: از داده‌های رفرنس برای انتخاب یک مقدار واقعی و قطعی برای این بیمار خاص استفاده کنید.
        5.  در تولید شرح حال فقط مجاز به اشاره به داروهای OTC داری و تنها لیست داروها را بده
        6.  در تولید سناریو به بیماری های خانواده که با این بیماری مرتبط است اشاره مستقیم نکن
        7.  در تولید سناریو به سابقه علائم بیماری های مرتبط با بیماری {disease} اشاره نکن
